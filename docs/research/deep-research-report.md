...
`harness-auto-test-evidence` を、単なる「テスト結果の収集器」ではなく、自動テスト証跡の信頼性を評価し、その評価結果を Quality Evidence Graph に接続して Gate 判定まで一貫させるハーネスとして再定義するのが最も妥当です。既存の RNA4219 リポジトリ群はすでに、手動観点生成 (`manual-bb-test-harness`)、差分とリスクの抽出 (`code-to-gate`)、証跡グラフと DQ 優先ガバナンス (`quality-evidence-graph`)、Evidence 契約 (`agent-protocols`)、実行統制 (`shipyard-cp` / `agent-state-gate` / `agent-gatefield`) という骨格を持っています。HATE の役割は、この骨格に対して 自動テスト固有の証跡収集・正規化・評価・可視化・外部エコシステム接続を埋めることです。
...
追加機能としては、`HATE explain` と gap recommendation が有用です。`HATE explain --risk RISK-001` のように、特定 risk や test case がなぜ `go`, `conditional_go`, `no_go`, `disqualified` になったかを辿れると、summary だけでは分からない Gate の説明責任を満たせます。また high-risk path に証跡が不足した場合、追加すべき test layer、manual 補完要否、Pact / mutation / Playwright などの推奨証跡を出せると、HATE は単なる判定器ではなく改善ループの起点になります。
...
`agent-protocols`、`workflow-cookbook`、`shipyard-cp`、`agent-state-gate`、`agent-gatefield` は、HATE が後段で publish / human review / approval binding に接続するための基盤です。とくに `agent-state-gate` が approval を `diff_hash` と `context_hash` に束縛して古い承認の再利用を防ぐ点は、HATE の provenance 設計でもそのまま活かすべきです。HATE が出す Gate 判定は、単発 JSON ではなく、後段で再演算・監査できる binding 付き packet である必要があります。
...
| FR-10 | DQ ルールを評価し、失格証跡を gate 計算から除外または全体失格にする |
...
実装優先度は、FR 単位で次のように固定するのが安全です。P0 は local-first で gate 判定まで閉じる最小範囲、P1 は証跡信頼度を上げる拡張、P2 は外部運用・可視化との接続です。

| 優先度 | 対象 FR | 実装方針 |
|---|---|---|
| P0 | FR-01, FR-02, FR-03, FR-05, FR-06, FR-08, FR-10, FR-11, FR-12, FR-14 | GitHub Actions provenance、JUnit XML、LCOV/Cobertura/JaCoCo、Playwright artifact、SARIF、QEG export、record を最小実装する |
| P1 | FR-04, FR-07, FR-09, FR-15, FR-16, FR-17, FR-18 | Pact / Stryker / coverage.py contexts / AETE / agent-protocols 写像、RanD 要件監査接続、shipyard-cp run/audit 接続、workflow-cookbook 実装運用接続を追加し、信頼評価と説明可能性を強化する |
| P2 | FR-13 | Allure / ReportPortal / Codecov / SonarQube export は non-gating optional adapter とし、外部サービス未導入でも gate 判定を妨げない |
...
| `gate-decision.json` | verdict / DQ / thresholds / reasons |
...
推奨 exit code は以下です。これは QEG の現在ルールを踏襲するのが自然です。

| exit code | 意味 |
|---|---|
| 0 | `go` |
| 1 | CLI / schema / adapter failure |
| 2 | `conditional_go` / `no_go` / `disqualified` |
...
| DQ ID | 条件 | 既定 severity | gate への影響 |
|---|---|---|---|
| HATE-DQ-01 | commit SHA が不一致、または欠落 | hard_dq | run 全体を `disqualified` |
| HATE-DQ-02 | schema invalid / parse failed | hard_dq | 対象 artifact を除外。必須 artifact なら run 全体を `disqualified` |
| HATE-DQ-03 | artifact hash 不在または参照ファイル欠落 | hard_dq | 対象 evidence を除外。必須 artifact なら run 全体を `disqualified` |
| HATE-DQ-04 | timestamp が run window 外 | hard_dq | 対象 evidence を除外。主要証跡なら `no_go` |
| HATE-DQ-05 | flaky が policy 閾値超過、または unresolved | hard_dq | 高リスク対象なら `no_go`、低リスク対象なら evidence 除外 |
| HATE-DQ-06 | coverage が source file に解決できない | soft_gap | coverage evidence を除外 |
| HATE-DQ-07 | changed high-risk path に対応テストが 0 | hard_dq | `no_go` または manual 補完要求 |
| HATE-DQ-08 | coverage だけあり execution result がない | hard_dq | coverage evidence を除外。唯一証跡なら `no_go` |
| HATE-DQ-09 | Pact `can-i-deploy` が negative | hard_dq | 契約変更を含む場合は `no_go` |
| HATE-DQ-10 | changed path に critical/high SARIF unresolved | hard_dq | `no_go` |
| HATE-DQ-11 | mutation required path で mutation evidence 欠落 | soft_gap | AETE を減点。release profile では `conditional_go` |
| HATE-DQ-12 | browser / platform matrix 必須なのに不足 | soft_gap | AETE を減点。必須 matrix なら `conditional_go` |
| HATE-DQ-13 | manual override に approval binding 不足 | hard_dq | override を無効化し、元 verdict を維持 |
| HATE-DQ-14 | report が stale branch / stale baseline に基づく | hard_dq | run 全体を `disqualified` |
| HATE-DQ-15 | own-output validation の record が生成されない | hard_dq | run 全体を `disqualified` |

`hard_dq` は証跡として採用しない状態、`soft_gap` は採用可能だが AETE / verdict を下げる状態、`warning` は summary に表示するが gate には直接影響しない状態です。MVP では `hard_dq` と `soft_gap` の二段階だけでもよいですが、JSON 上は `severity` を必須にして将来拡張できるようにします。
...
| Freshness / Policy Conformance | 5 | stale でないこと、policy 適合 |
...
| 0–59 | 信頼不足。gate 計算で block/hold 相当 |
| DQ | 計算対象外または全体失格 |
...
| Freshness / Policy Conformance | stale または policy 不適合 | baseline が古い | run window 内で profile に概ね適合 | run window 内かつ profile 必須条件を全て満たす |
...
```json
{
  ...
  "redaction_status": "not_required",
  "payload": {
    "decision": "go|conditional_go|no_go|disqualified",
...
| FR-10 gate / DQ | `gate-decision.json` | `GateDecision`, `disqualified_by` |
...
| 優先 | 範囲 | 理由 |
|---|---|---|
| P0 | GitHub Actions provenance、JUnit XML、Playwright trace/screenshot/video、LCOV/Cobertura/JaCoCo、SARIF、QEG export | 多言語 CI を最短でまとめるため |
| P1 | coverage.py contexts、Vitest/Jest adapters、Pact、Stryker、OpenTelemetry、Allure export、RanD requirements audit alignment、shipyard-cp run evidence attachment、workflow-cookbook artifact mapping | 信頼評価と explainability を強化するため |
| P2 | ReportPortal、Codecov、SonarQube、agent-state-gate / gatefield 深統合 | チーム運用・長期履歴・上位統制へ広げるため |

### 段階ロードマップ

MVP-1 では、`collect → normalize → correlate → gate → qeg export` の最短ループを完成させます。ここで必要なのは `GitHub metadata adapter`, `JUnit XML adapter`, `coverage adapter`, `SARIF adapter`, `artifact-manifest`, `ctg reader`, `qeg exporter`, `summary.md`, `job summary` です。これだけで PR ごとの“証跡付き自動品質保証”は成立します。
...
MVP-2 の同じ段階で、RanD と shipyard-cp への接続も追加すると客観性が上がります。RanD 接続では `requirements_audit_packet.json` を fixture 入力にして、requirement ごとの `testability`, `implementation_alignment`, `gate_verdict` と HATE evidence の対応を検証します。shipyard-cp 接続では `RunSystemPacket` / `WorkerResult` fixture に HATE artifact refs、AETE summary、DQ summary を添付し、Shipyard の advisory run/audit evidence として保存可能な JSON を生成します。

加えて、workflow-cookbook 接続では HATE-MVP-* を Task Seed 互換に展開し、acceptance record と evidence refs へ辿れる `workflow-*` artifact を生成します。これにより、HATE の実装作業は「どのタスクが、どの検収条件を、どの証跡で満たしたか」を cookbook の運用単位で確認できます。Birdseye / docs stale の情報は HATE 自身の判定ではなく、workflow-cookbook / memx-resolver 側へ渡す freshness 素材として扱います。
